...
| 9 | Mobil uygulama yok — read-only yönetici izleme uygulaması gerekiyor (web ile aynı auth/backend) | `app/m/`, `lib/mobile/`, `app/api/mobile/` | **Faz 5** | ✅ Tamamlandı — `/m` mobil izleme paneli (KPI, kanal, uyarı), Bearer token API (login/logout/restaurants/overview), `buildMobileOverview` + 7 test, sidebar'da "Mobil izleme" linki |
