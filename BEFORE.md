...
| 9 | Mobil uygulama yok — read-only yönetici izleme uygulaması gerekiyor (web ile aynı auth/backend) | `app/m/`, `lib/mobile/`, `app/api/mobile/`, `mobile/` | **Faz 5** | ✅ Tamamlandı — (1) Web: `/m` mobil izleme paneli (cookie auth, server-rendered KPI/kanal/uyarı), sidebar'da link. (2) Native: React Native Expo uygulaması (Bearer token auth, SecureStore, pull-to-refresh). (3) Backend: login/logout/restaurants/overview API, rate limiting (15dk/10 deneme). (4) Test: `buildMobileOverview` + 7 birim test |
| 10 | Kalite altyapısı yok — lint/test yok, CI sadece build | `.github/workflows/ci.yml`, `package.json`, `eslint.config.mjs` | **Faz 7** | ✅ Tamamlandı — ESLint 9 flat config (`eslint.config.mjs`) + `npm run lint` script. Vitest 75 test / 9 dosya (engine, ingestion, locale, i18n, session, mobile-overview, mobile-contract, middleware, app-language). CI pipeline: lint → typecheck → test → build. Kalan sadece e2e/integration testleri (ayrı bir dilim olarak planlanabilir) |
